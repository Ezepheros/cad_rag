...
from cad_rag.embedding_models.embedding_models_wrapper import EmbeddingModelWrapper, VoyageaiEmbeddingModelWrapper
import re
...
class NaiveIndexer(Indexer):
    ...
    def create_index(self):
        ...
        for i, doc in enumerate(self.dataset):
            ...
            if not self.breakpoints.get("exit_forloop", False):
                ...
                breakpoint()
            doc_metadata = {doc_key: doc[doc_key] for doc_key in doc if doc_key != 'unofficial_text_en'}
            ...
            if len(chunks) >= self.embeddings_per_request:
                ...
                for j in range(len(chunks)):
                    ...
                    table = pq.Table.from_pydict({k: [v] for k, v in chunk_record.items()})
    ...
    def get_knn(self, query: str, k: int = 5):
        ...
        if self.index_ is None:
            self.read_index()
        D, I = self.index_.search(np.array([query_emb]), k)
        ...
        filtered = dataset.to_table(filter=ds.field("idx").isin(index_id_set))
